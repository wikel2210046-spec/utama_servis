...
<div class="container">
    ...
    <!-- FILTER PEMBAYARAN / SERVIS -->
    <form action="<?= base_url('Pembayaran/laporan') ?>" method="get" class="filter-bar bg-light p-3 rounded shadow-sm mb-4">
        <div class="row align-items-end g-2">
            ...
            <div class="col-md-2 col-sm-6 d-flex justify-content-end align-items-center mb-2 mb-md-0" style="gap: 4px;">
                ...
                <a href="<?= base_url('Pembayaran/laporan') ?>" class="btn btn-secondary btn-sm shadow-sm" title="Reset Filter"><i class="fa-solid fa-arrows-rotate"></i></a>
            </div>
        </div>
    </form>
            ...
            <?php if (($periode ?? '') == 'tahun'): ?>
                ...
                <tbody>
                    ...
                    foreach ($months as $num => $namaBulan): 
                        $dt = $dataBulan[$num] ?? ['jumlah_transaksi' => 0, 'total_bayar' => 0];
                    ?>
                        <tr>
                            <td class="text-center"><?= $no++; ?></td>
                            <td class="text-center"><?= esc($namaBulan) ?></td>
                            <td class="text-center"><?= esc($dt['jumlah_transaksi']) ?></td>
                            <td class="text-end"><?= number_format($dt['total_bayar'], 0, ',', '.') ?></td>
                        </tr>
                    <?php endforeach; ?>
                    <tr class="table-info">
                        <td colspan="3" class="text-end"><strong>Total Pemasukan (Rp)</strong></td>
                        <td class="text-end"><strong><?= number_format($total_keseluruhan, 0, ',', '.') ?></strong></td>
                    </tr>
                </tbody>

    ...
    <div class="d-flex justify-content-between align-items-end mb-2">
        <div>
            ...
            if (!empty($sumber_terpilih)) {
                $filterTeks[] = "Sumber: " . ($sumber_terpilih == 'pembayaran' ? 'Pembayaran Servis' : 'Penjualan Sparepart');
            } else {
    ...
    <form action="<?= base_url('Laporanuangmasuk/filter') ?>" method="get" class="filter-bar bg-light p-3 rounded shadow-sm mb-4">
        <div class="row align-items-end g-2">
            ...
            <div class="col-md-2 col-sm-4 d-flex justify-content-end align-items-center mb-2 mb-md-0" style="gap: 4px;">
                ...
                <a href="<?= base_url('Laporanuangmasuk/index') ?>" class="btn btn-secondary btn-sm shadow-sm" title="Reset Filter"><i class="fa-solid fa-arrows-rotate"></i></a>
            </div>
        </div>
    </form>
        ...
        <table class="table table-bordered table-striped align-middle">
            <?php if (($periode ?? '') == 'tahun'): ?>
                ...
                <tbody>
                    ...
                    foreach ($months as $num => $namaBulan): 
                        $dt = $dataBulan[$num] ?? ['jumlah_transaksi' => 0, 'total_pemasukan' => 0];
                    ?>
                        <tr>
                            <td class="text-center"><?= $no++; ?></td>
                            <td class="text-center"><?= esc($namaBulan) ?></td>
                            <td class="text-center"><?= esc($dt['jumlah_transaksi']) ?></td>
                            <td class="text-end"><?= number_format($dt['total_pemasukan'], 0, ',', '.') ?></td>
                        </tr>
                    <?php endforeach; ?>
                    <tr class="table-success">
                        <td colspan="3" class="text-end"><strong>Total Uang Masuk</strong></td>
            ...
            <?php elseif (($periode ?? '') == 'bulan'): ?>
                ...
                <tbody>
                    <?php if (!empty($dataUangMasuk)): ?>
                        ...
                        ?>
                            <tr>
                                ...
                                <td class="text-end"><?= number_format($row['total_pemasukan'], 0, ',', '.') ?></td>
                            </tr>
                        <?php endforeach; ?>
                        <tr class="table-success">
                            <td colspan="3" class="text-end"><strong>Total Uang Masuk</strong></td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="4" class="text-center">Tidak ada data uang masuk.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            <?php else: ?>
                ...
                <tbody>
                    <?php if (!empty($dataUangMasuk)): ?>
                        <?php $no = 1; foreach ($dataUangMasuk as $row): ?>
                            <tr>
                                <td class="text-center"><?= $no++; ?></td>
                                <td class="text-center"><?= date('d-m-Y', strtotime($row['tanggal'])) ?></td>

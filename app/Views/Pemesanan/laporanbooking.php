...
<div class="container">
    ...
    <!-- FILTER BOOKING -->
    <form action="<?= base_url('Pemesanan/filterLaporan') ?>" method="get" class="filter-bar bg-light p-3 rounded shadow-sm mb-4">
        <div class="row align-items-end g-2">
            <div class="col-md-4 col-sm-6 mb-2 mb-md-0" id="slot-1">
            ...
            <div class="col-md-4 col-sm-6 mb-2 mb-md-0" id="slot-2">
            ...
            <div class="col-md-2 col-sm-6 mb-2 mb-md-0">
                ...
                <select name="periode" id="select-periode" class="form-control form-control-sm" onchange="toggleFilterFields()">
                    ...
                    <option value="hari" <?= ($periode ?? 'hari') == 'hari' ? 'selected' : '' ?>>Per Hari</option>
                    <option value="bulan" <?= ($periode ?? '') == 'bulan' ? 'selected' : '' ?>>Per Bulan</option>
                    <option value="tahun" <?= ($periode ?? '') == 'tahun' ? 'selected' : '' ?>>Per Tahun</option>
                </select>
            </div>

            <div class="col-md-2 col-sm-6 d-flex justify-content-end align-items-center mb-2 mb-md-0" style="gap: 4px;">
                ...
                <a href="<?= base_url('Pemesanan/laporan') ?>" class="btn btn-secondary btn-sm shadow-sm" title="Reset Filter"><i class="fa-solid fa-arrows-rotate"></i></a>
            </div>
        </div>
    </form>
        ...
        <table class="table table-bordered table-striped align-middle">
                ...
                <tbody>
                    ...
                    if (!empty($pemesanan)) {
                        foreach ($pemesanan as $row) {
                            $dataBulan[(int)$row['bulan']] = $row['jumlah_booking'];
                        }
                    }
                    foreach ($months as $num => $namaBulan): 
                        $jumlah = $dataBulan[$num] ?? 0;
                    ?>
                        <tr>
                    ...
                    <tr class="table-info">
                        ...
                        <td class="text-center"><strong><?= esc($total_booking) ?> Booking</strong></td>
                    </tr>
                </tbody>
            <?php elseif (($periode ?? '') == 'bulan'): ?>
                ...
                <tbody>
                    <?php if (!empty($pemesanan)): ?>
                        ...
                        ?>
                            <tr>
                                ...
                                <td class="text-center"><?= esc($row['jumlah_booking']) ?></td>
                            </tr>
                        <?php endforeach; ?>
                        <tr class="table-info">
                            <td colspan="2" class="text-end"><strong>Total Keseluruhan</strong></td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="3" class="text-center">Tidak ada data booking.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            <?php else: ?>
                ...
                <tbody>
                    <?php if (!empty($pemesanan)): ?>
                        ...
                        <?php endforeach; ?>
                        <tr class="table-info">
                            <td colspan="7" class="text-end"><strong>Total Keseluruhan</strong></td>
                            <td class="text-center"><strong><?= esc($total_booking) ?> Booking</strong></td>
                        </tr>
                    <?php else: ?>
                        <tr>
